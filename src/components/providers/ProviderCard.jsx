...
export default function ProviderCard({ provider }) {
  const providerId = provider.id || provider.user_id;
  ...
  return (
    <div className="card-dark p-6 hover:border-[#FF6633]/30 transition-all duration-300 animate-fade-in">
      <div className="flex flex-col items-center text-center">
        ...
        <div className="relative mb-4">
          <div className="w-24 h-24 rounded-full p-1 bg-gradient-to-br from-[#7CB342] to-[#689F38]">
            <div className="w-full h-full rounded-full overflow-hidden bg-[#1A1D2E]">
              {provider.avatar_url ? (
                <img src={provider.avatar_url} alt={providerName} className="w-full h-full object-cover" />
              ) : (
        ...
        {/* Name */}
        <h3 className="text-lg font-semibold text-white mb-1">{providerName}</h3>
        ...
        {/* View Profile Button */}
        <Link
          to={createPageUrl(`ProviderProfile?id=${providerId}`)}
