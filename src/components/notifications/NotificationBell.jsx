...
export default function NotificationBell({ userEmail }) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-80 p-0 bg-[#1A1D2E] border-[#2A2D3E]">
        <div className="flex items-center justify-between p-4 border-b border-[#2A2D3E]">
          <h3 className="text-white font-semibold">Notifications</h3>
          {unreadCount > 0 && (
            <button onClick={markAllAsRead} className="text-[#FF6633] text-sm hover:underline">
              Mark all read
            </button>
          )}
        </div>
            ...
            notifications.map(notification => (
              ...
              >
                <div className="flex gap-3">
                  <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${typeColors[notification.type] || typeColors.system}`} />
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm ${notification.is_read ? 'text-gray-400' : 'text-white font-medium'}`}>
                      {notification.title}
                    </p>
        ...
        <div className="p-3 border-t border-[#2A2D3E]">
          <Link to={createPageUrl('DashboardNotifications')} onClick={() => setOpen(false)}>
